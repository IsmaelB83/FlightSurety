...
contract FlightSuretyData {
    ...
    /********************************************************************************************/
    function registerFirstAirline (address account, string memory name) external requireIsOperational requireContractOwner {
        require(airlinesArray.length == 0, "Only first airline can be registered here");
        airlines[account] = Airline(name, true, true, false, 0);
        airlinesArray.push(account);
    }

    function registerAirline (address account, string memory name) external requireIsOperational requireAuthorizedContract {
        require(!airlines[account].isRegistered, "Airline already registered");
        airlines[account] = Airline(name, true, false, false, 0);
    }
