...
contract('FlightSuretyApp and FlightSuretyData Tests', async (accounts) => { 
    ...
    it('Can block access to functions using requireIsOperational when operating status is false', async function () {
        let error = false;
        try  {
            await flightSuretyApp.registerAirline(airline_2, "Emirates", {from: airline_1});
        }
        catch(e) {
            error = true;
        }
    ...
    it('Founder airline cannot register additional airlines if it has not provided funding', async () => {
        ...
        let result = await flightSuretyData.isFundedAirline(airline_2);
    ...
    it('First airline can register only 3 additional airlines.', async () => {
        await flightSuretyApp.registerAirline(airline_3, "Qatar", { from: airline_1 });
        await flightSuretyApp.registerAirline(airline_4, "British Airways", { from: airline_1 });
        let exception = false;
        try {
            await flightSuretyApp.registerAirline(airline_5, "Japan Airlines", { from: airline_1 });
        } catch (e) {
            exception = true;
        }
    ...
    it('Fifth airline requires voting to be accepted', async () => {
        await flightSuretyApp.registerAirline(airline_5, "Japan Airlines", { from: airline_5 });
    ...
    it('An accepted airline can register flights', async () => {
        ...
        await flightSuretyApp.registerFlight("BA5410", timestamp, {from: airline_1})
        assert.equal((await flightSuretyData.getFlightInfo(airline_1, "IB3410", timestamp)).flightAirline, airline_1, "Flight not registered properly");
        assert.equal((await flightSuretyData.getFlightInfo(airline_1, "IB3410", timestamp)).flightAirline, airline_1, "Flight not registered properly");
    });

    it('Passenger can purchase insurance', async () => {
        let flights = await flightSuretyData.getFlights();
        await flightSuretyData.buy(flights[0], {from: passenger_1, value: web3.utils.toWei('1', 'ether')})
        await flightSuretyData.buy(flights[0], {from: passenger_2, value: web3.utils.toWei('1', 'ether')})           
        let aux = await flightSuretyData.getInsurances(flights[0]);
